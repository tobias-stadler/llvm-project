...
/// limiting and non-obvious for selector implementers. Therefore, to allow
/// deletion of arbitrary instructions, we now detect this case and continue
...
public:
  ...
  MIIteratorMaintainer(RetryListTy &WorkList) : RetryList(WorkList) {}
  ...
  void MF_HandleRemoval(MachineInstr &MI) override {
    ...
    if (!RetryList.empty()) {
      // WorkList is rarely non-empty, so only perform expensive Map lookup when
      // needed.
      RetryList.remove(&MI);
    }
...
// TODO: final documentation after deciding if this is the way to go
class InstructionSelect::RetryListMaintainer : public GISelChangeObserver {
  RetryListTy &RetryList;

public:
  RetryListMaintainer(RetryListTy &RetryList) : RetryList(RetryList) {}

  void erasingInstr(MachineInstr &MI) override { RetryList.remove(&MI); }

  void changingInstr(MachineInstr &MI) override {}

  void changedInstr(MachineInstr &MI) override {
    LLVM_DEBUG(dbgs() << "Retry due to change:   " << MI);
    RetryList.insert(&MI);
  }

  void createdInstr(MachineInstr &MI) override {
    LLVM_DEBUG(dbgs() << "Retry due to creation: " << MI);
    RetryList.insert(&MI);
  }
};
...
bool InstructionSelect::runOnMachineFunction(MachineFunction &MF) {
  ...
  RetryList.clear();
  MIIteratorMaintainer MIIMaintainer(RetryList);
  // TODO: decide where to inject this
  RetryListMaintainer RLMaintainer(RetryList);

  {
    // Observe IR insertions and removals during selection.
    // We only install a MachineFunction::Delegate instead of a
    // GISelChangeObserver, because we do not want notifications for changed
    // instructions to prevent significant compile-time regressions from e.g.
    ...
    for (MachineBasicBlock *MBB : post_order(&MF)) {
      ...
      for (auto End = MBB->rend(); MIIMaintainer.MII != End;) {
        ...
        // Select instructions that have been marked for reselection.
        while (!RetryList.empty()) {
          MachineInstr &MI = *RetryList.pop_back_val();
          assert(MI.getParent() && "RetryList contained unlinked MI");
          ISel->CurMBB = MI.getParent();
          LLVM_DEBUG(dbgs() << "\nSelect during retry:  " << MI);
          if (!select(*ISel, MI)) {
            LLVM_DEBUG(dbgs() << "Selection failed during retry!\n";
                       MIIMaintainer.reportFullyCreatedInstrs(););
            reportGISelFailure(MF, TPC, MORE, "gisel-select", "cannot select",
                               MI);
            return false;
...
bool InstructionSelect::select(InstructionSelector &ISel, MachineInstr &MI) {
