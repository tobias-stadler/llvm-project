...
static cl::SubCommand FilterSub(
    "filter",
...
public:
  ...
  Error handleInputFile(StringRef InputFileName) {
  ...
  void finalize() {
    if (!Serializer)
      return;
    OF->keep();
    auto Defer = make_scope_exit([&]() { Serializer = nullptr; });
    if (!Sort && !Dedupe)
      return;
...
private:
  ...
  /// For now, use std::map (following RemarkLinker) for easy sorting. We
  /// should probably be using some combination of a trie and hashing,
  /// capitalizing on the fact that the strings are already interned.
  std::map<std::unique_ptr<Remark>, size_t> Remarks;
  StringTable StrTab;
...
static Error tryFilter() {
  ...
  for (auto &InputFileName : InputFileNames) {
    if (Error E = Tool.handleInputFile(InputFileName))
      return E;
  }
  return Error::success();
}
